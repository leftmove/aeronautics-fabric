...
//        debug.popDebugGroup();
    ...
    private static void renderLayers(final EndSeaPhysics physics, final Camera camera) {
        ...
        shader.addSampler("ShadowDepthSampler", shadowBuffer.getDepthTextureAttachment().getId());
        shader.addSampler("ShadowStrengthSampler", shadowBuffer.getColorTextureAttachment(0).getId());
        ...
        shader.setFloat("ShadowVolumeSize", EndSeaShadowRenderer.SHADOW_VOLUME_RADIUS);
        shader.setFloat("StartY", (float) physics.startY());

        final LocalPlayer player = minecraft.player;
        final float renderTime = player.tickCount + Minecraft.getInstance().getFrameTime();
